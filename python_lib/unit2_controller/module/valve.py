...
class ValveObj:
    ...
    power_output: isotope.port.PowerOutput
    power: int
    normally_open: bool
    ...
    def initialise(self, board: isotope.Isotope_comms_protocol, port_id: int) -> None:
        ...
        """
        pwm = round(self.power * 1024 / 100)
        self.motor = isotope.port.PowerOutput(board, port_id, pwm)
...
class Valve:
    ...
    def _configure(self):
        ...
        for device in self._config['devices']:
            valve = ValveObj()
            valve.power = device.get('power', defaults['power'])
            valve.normally_open = device.get('normally_open', defaults['normally_open'])
            valve.initialise(self._boards[device['board_id']], device['port_id'])
